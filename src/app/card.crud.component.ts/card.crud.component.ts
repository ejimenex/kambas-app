...
export class CardCrudComponent implements OnInit {
 ...
 this.localStorageName=this.isToDo?localStorageName.toDo:localStorageName.inProgress;
  }
  close() {
    this.activeModal.close();
  }
  validateAllField():boolean{
    if(!this.card.title || !this.card.description || !this.card.dueDate || !this.card.tag || !this.card.asignedTo)
    return false;
    return true;
  }
  add() {
  ...
  edit() {
    ...
    this.alertService.questionSWA(()=>{
        this.cardService.editCard(this.card,this.indexToEdit,this.localStorageName);
        this.alertService.success('Card mofified sucessfully');
        this.notifyParent.emit();
        this.close();

    },'Confirmation','Are you sure you want to edit the card?')
   
  }
  removeToDo() {
    this.alertService.questionSWA(
      () => {
        this.cardService.removeCardForm(this.indexToEdit,this.localStorageName);
  ...
  addOrEdit(){
    if(!this.validateAllField()) return this.alertService.error('Please, fill all fields!')
